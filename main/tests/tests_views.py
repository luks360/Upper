...
class TestViews(TestCase):
    def setUp(self):
        ...
        self.profit = Profits.objects.create(
            id=1, name='test', value=20, group=self.group, date=datetime.date(2013, 12, 3), user=user)

        self.groupS = GroupSpending.objects.create(name='testando', user=user)
        self.spending = Spending.objects.create(
            id=1, name='test', value=20, group=self.groupS, date=datetime.date(2013, 12, 3), user=user)
    ...
    def test_profit_edit_url(self):
        response = self.client.post(
            reverse('profitEdit', kwargs={'id': 1}), {'name': 'name', 'value': 20, 'date': datetime.date(2012, 12, 20), 'group': self.groupS}, follow=True)
        self.assertEqual(response.status_code, 200)

    def test_profit_edit_data_none_url(self):
        response = self.client.get(
            reverse('profitEdit', kwargs={'id': 1}), follow=True)
        self.assertEqual(response.status_code, 200)
    ...
    def test_spending_edit_url(self):
        response = self.client.post(
            reverse('spendingEdit', kwargs={'id': 1}), {'name': 'name', 'value': 20, 'date': datetime.date(2012, 12, 20), 'group': self.groupS}, follow=True)
        self.assertEqual(response.status_code, 200)
    ...
    def test_spending_create_url(self):
        response = self.client.post(
            reverse('spending'), {'name': 'name', 'value': 20, 'date': datetime.date(2012, 12, 20), 'group': self.groupS}, follow=True)
        self.assertEqual(response.status_code, 200)

    def test_profit_create_url(self):
        response = self.client.post(
            reverse('profits'), {'name': 'name', 'value': 20, 'date': datetime.date(2012, 12, 20), 'group': self.group}, follow=True)
        self.assertEqual(response.status_code, 200)
